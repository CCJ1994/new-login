...
<div class="container">
  ...
  <form action="save_user.php" method="post" class="col-md-6">
    <ul class="list-group">
      ...
      <li class="list-group-item">密碼：<input type="text" name="pw" value="<?=$user['pw']?>"></li>
      <li class="list-group-item">姓名：<input type="text" name="name" value="<?=$user['name']?>"></li>
      <li class="list-group-item">生日：<input type="text" name="birthday" value="<?=$user['birthday']?>"></li>
      ...
      <li class="list-group-item">角色：
        <select name="role" id="">
          ...
          <option value="管理員" <?=($user['role']=='管員')?"selected":"";?>>管理員</option>
        </select>
      </li>
    </ul>
